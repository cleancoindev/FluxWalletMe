...
function initialize() {
    web3.eth.getBlockNumber(function (err, lastSeenBlock) {
        ...
        wrappedEtherContract.Wrapped(changeHandler); // The Wrapped event tells us when the number of wrapped ETC changes.
        wrappedEtherContract.Unwrapped(changeHandler); // The Unrapped event tells us when the number of wrapped ETC changes.
    });
}
...
function loadData(blockNumber) {
    ...
    function updateWhenDone() {
        if (currentSupply !== undefined && wrappedEther !== undefined && unwrappedEther !== undefined) {
            console.log("There is " + web3.fromWei(currentSupply) + " ETC wrapped at present.");
            console.log(web3.fromWei(wrappedEther) + " ETC has been wrapped during the lifetime of this contract.");
            console.log(web3.fromWei(unwrappedEther) + " ETC has been unwrapped during the lifetime of this contract.");
            ...
            $('#userWETCBalance').html('<span class="text-warning">' + web3.fromWei(etcBalance) + '</span>');
